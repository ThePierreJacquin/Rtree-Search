 ...
 */
int RTreeSearch( RTREENODE *node, RTREEMBR *rc, pfnSearchHitCallback pfnSHCB, struct Vertex* point, struct Mesh* mesh)
{
    /* Fix not yet tested. */
    int hitCount = 0;
    int i;
    ...
    if (node->level > 0) /* this is an internal node in the tree */
    {
        for (i=0; i<NODECARD; i++){
            if (node->branch[i].child && RTreeOverlap(rc, &node->branch[i].mbr))
                hitCount += RTreeSearch(node->branch[i].child, rc, pfnSHCB, point, mesh);
        }
    }
...
#pragma warning( disable : 4311 )
        for (i=0; i<LEAFCARD; i++)
        {
			...
			if(pfnSHCB && ! pfnSHCB((int)node->branch[i].child, point, mesh) )
								return hitCount; 
            }
        }
#pragma warning(pop)
